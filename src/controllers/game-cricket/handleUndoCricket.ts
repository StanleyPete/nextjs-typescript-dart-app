...
export const handleUndoCricket = (
   ...
   const lastThrowSector: '20' | '19' | '18' | '17' | '16' | '15' | 'Bull' | '' = lastEntry.historyLastThrowSector
   ...
   else if (currentPlayerThrowsCount === 0 && currentPlayerOrTeam.legs > lastEntry.historyLegs){
      currentPlayerOrTeam.legs -= 1
      dispatch(setCurrentPlayerThrowsCount(lastEntry.historyThrows.length))

      gamePlayersOrTeams.forEach((playerOrTeam: PlayerCricket | TeamCricket, index: number) => {
        if('historyPlayerIndex' in lastEntry){
            if (index !== lastEntry.historyPlayerIndex) {
               const playerLastHistoryEntry = [...history as HistoryEntryCricketSingle[]]
                  .reverse()
            ...
            if (index !== lastEntry.historyTeamIndex) {
                const teamLastHistoryEntry = [...history as HistoryEntryCricketTeams[]]
                   .reverse()
   ...
   const isAnyPlayerOrTeamWhichHaveNotCompletedSector = playersOrTeams.some(playerOrTeam => playerOrTeam.scores[lastThrowSector] !== 3)
