...
export const handleSubmitScoreButton = (
   ...
   //Scenario when player has already thrown at least once, but NOT 3 times
      ...
      if (lastThrow === '25' || lastThrow === '50') {
         ...
         sector = lastThrow.replace(/[^0-9]/g, '') as '20' | '19' | '18' | '17' | '16' | '15'
      }
   ...
   if (gameType === 'single'){
      dispatch(setHistoryCricketSingle([...history as HistoryEntryCricketSingle[], newHistoryEntry as HistoryEntryCricketSingle]))
   } else {
      dispatch(setHistoryCricketTeams([...history as HistoryEntryCricketTeams[], newHistoryEntry as HistoryEntryCricketTeams]))
   }
   handleSwitchPlayerOrTeamCricket(gameType, index, currentPlayerIndexInTeam, playersOrTeams, dispatch)
   dispatch(setCurrentPlayerThrowsCount(0))
   dispatch(setCurrentPlayerThrows([]))
